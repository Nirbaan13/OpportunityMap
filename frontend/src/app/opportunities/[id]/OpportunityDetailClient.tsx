...
type OpportunityDetailClientProps = {
  ...
  initialOpportunity: OpportunityDetail | null;
};
...
export function OpportunityDetailClient({
  ...
  const [error, setError] = useState<string | null>(
    initialOpportunity ? null : "Opportunity not found.",
  );
  const [loading, setLoading] = useState(!initialOpportunity);

  useEffect(() => {
    setOpportunity(initialOpportunity);
    setError(initialOpportunity ? null : "Opportunity not found.");
    setLoading(false);
  }, [initialOpportunity]);

  useEffect(() => {
    if (initialOpportunity || !Number.isFinite(id) || id <= 0) return;
